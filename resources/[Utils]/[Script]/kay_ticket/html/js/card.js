...
$(function() {
  window.addEventListener('message', function(event) {
    if (event.data.type === "openScratch") {
      ...
      if(price_type == 'money') {
        formattedPrice = currency + ' ' + Number.parseFloat(price).toFixed(0);
        price > 0 ? document.getElementById('price').innerHTML = "<span style='color:#FFF'>Vous avez Gagné ! 😁</span>" : document.getElementById('price').innerHTML = "<span style='color:#FFF'>Vous avez Perdu ! 😔 </span>";
      } else {
        document.getElementById('price').innerHTML = "<span style='color:#FFF'>Vous avez Gagné ! 😁</span>"
      }
