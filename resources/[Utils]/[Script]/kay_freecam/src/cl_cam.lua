...
RegisterNetEvent('admin:toggleCamera', function(state)
    -- state:
    -- true  => activer
    -- false => désactiver
    if state == false then
    ...
    -- reset zoom target (propre)
    if FreeCam and FreeCam.Fovs and FreeCam.Fovs.TargetFov then
        FreeCam.Fovs.TargetFov = currentFov
    end
    ...
    Citizen.CreateThread(function()
        while inCamera do
            ...
            -- IMPORTANT: disable controls en boucle
            ...
            local controlsToDisable = {30, 31, 32, 33, 34, 35} -- move axes
            ...
            if zoomInput ~= 0 then
                FreeCam.Fovs.TargetFov = math.max(
                    FreeCam.Fovs.MinFov,
                    math.min(FreeCam.Fovs.MaxFov, FreeCam.Fovs.TargetFov + (zoomInput * FreeCam.Fovs.ZoomSpeed))
                )
            end
