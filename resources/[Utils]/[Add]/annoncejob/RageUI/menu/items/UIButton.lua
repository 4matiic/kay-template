local SettingsButton = {
    ...
    Text = { X = -12, Y = 9.5, Scale = 0.25 },
    LeftBadge = { Y = 30, Width = 37, Height = 37 },
    RightBadge = { X = 375, Y = 10, Width = 30, Height = 29 }, 
    RightText = { X = 425, Y = 0, Scale = 0.20 },
...
function RageUI.Button(Label, Description, Style, Enabled, Action, Submenu)
    ...
    if CurrentMenu ~= nil and CurrentMenu() then
        ...
        if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
            ...
            CurrentMenu.Y + 5 + SettingsButton.SelectedSprite.Y + CurrentMenu.SubtitleHeight + RageUI
            ...
            if Enabled then
                ...
                RenderText(not Active and Label or Label, CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset + 28, 5 + CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, fontIdButton, SettingsButton.Text.Scale, textR, textG, textB, 255);
            else
                if haveRightBadge then
                    local RightBadge = RageUI.BadgeStyle.Lock(Active)
                    RenderSprite(RightBadge.BadgeDictionary or "commonmenu", RightBadge.BadgeTexture or "", CurrentMenu.X + SettingsButton.RightBadge.X + CurrentMenu.WidthOffset + 7, CurrentMenu.Y + SettingsButton.RightBadge.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset + 6, 25, 25, 0, RageUI.Config.Menu.DisabledColor.R + 80, RageUI.Config.Menu.DisabledColor.G + 80, RageUI.Config.Menu.DisabledColor.B + 80, 255)
                end
                ...
                RenderText(Label,
                CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset + 28,
                5 + CurrentMenu.Y + SettingsButton.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset,
                fontIdButton,
                SettingsButton.Text.Scale,
                RageUI.Config.Menu.DisabledColor.R + 80,
                RageUI.Config.Menu.DisabledColor.G + 80,
                RageUI.Config.Menu.DisabledColor.B + 80,
                255)
            end
