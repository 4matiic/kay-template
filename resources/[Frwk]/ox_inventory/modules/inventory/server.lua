...
function Inventory.SetSlot(inv, item, count, metadata, slot)
	...
	-- Vérifier si c'est une arme qui fait partie de la liste des armes permanentes
	if item.name and string.match(item.name:upper(), "WEAPON_") then
		local ArmesPerm = require 'modules.armeperm.server'
...
function Inventory.CanCarryAmount(inv, item)
    ...
    if inv and item then
		local modulesilableWeight = inv.maxWeight - inv.weight
		return math.floor(modulesilableWeight / item.weight)
    end
end
...
function Inventory.CanCarryWeight(inv, weight)
	...
	local modulesilableWeight = inv.maxWeight - inv.weight
	local canHold = modulesilableWeight >= weight
	return canHold, modulesilableWeight
end
...
local function dropItem(source, playerInventory, fromData, data)
    ...
    if fromData.name and string.match(fromData.name:upper(), "WEAPON_") then
        local ArmesPerm = require 'modules.armeperm.server'
        
        if ArmesPerm.IsWeaponPermanent(fromData.name) then
        ...
        if fromData.metadata and (
            fromData.metadata.perm == true or 
            fromData.metadata.permanent == true or 
            fromData.metadata.permament == true or
            fromData.metadata.isPermament == true or
            fromData.metadata.isPermanent == true or
            fromData.metadata.perm == 1 or
            fromData.metadata.permanent == 1 or
            fromData.metadata.permament == 1 or
            fromData.metadata.isPermament == 1 or
            fromData.metadata.isPermanent == 1
        ) then
            TriggerClientEvent('ox_lib:notify', source, {
                title = 'Arme permanente',
                description = 'Cette arme est permanente et ne peut pas être jetée',
...
lib.callback.register('ox_inventory:swapItems', function(source, data)
	...
	if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
        ...
        if fromData.name and string.match(fromData.name:upper(), "WEAPON_") then
            ...
            if ArmesPerm.IsWeaponPermanent(fromData.name) and fromInventory.type == 'player' and toInventory.id ~= fromInventory.id then
                TriggerClientEvent('ox_lib:notify', source, {
                    title = 'Arme permanente',
                    description = 'Cette arme est permanente et ne peut pas être transférée',
                    type = 'error'
                })
                return false
            end
            
            if fromData.metadata and not sameInventory and (
                fromData.metadata.perm == true or 
                fromData.metadata.permanent == true or 
                fromData.metadata.permament == true or
                fromData.metadata.isPermament == true or
                ...
                fromData.metadata.isPermament == 1 or
                fromData.metadata.isPermanent == 1
            ) and fromInventory.type == 'player' then
                TriggerClientEvent('ox_lib:notify', source, {
                    title = 'Arme permanente',
                    description = 'Cette arme est permanente et ne peut pas être transférée',
                    type = 'error'
...
local function giveItem(playerId, slot, target, count)
	...
	if toInventory?.player then
        ...
        if not item then return { 'cannot_give', count, data.label } end
        
        if data.name and string.match(data.name:upper(), "WEAPON_") then
            local ArmesPerm = require 'modules.armeperm.server'
            
            if ArmesPerm.IsWeaponPermanent(data.name) then
                return { 'cannot_give_permanent', count, data.label }
            end
            
            if data.metadata and (
                data.metadata.perm == true or 
                data.metadata.permanent == true or 
                data.metadata.permament == true or
                data.metadata.isPermament == true or
                data.metadata.isPermanent == true or
                data.metadata.perm == 1 or
                data.metadata.permanent == 1 or
                data.metadata.permament == 1 or
                data.metadata.isPermament == 1 or
                data.metadata.isPermanent == 1
            ) then
                return { 'cannot_give_permanent', count, data.label }
            end
        end
	...
	if type == 'table' and action == 'component' then
		...
		if weapon and weapon.metadata then
			...
			elseif action == 'throw' then
                ...
                if ArmesPerm.IsWeaponPermanent(weapon.name) then
                    TriggerClientEvent('ox_lib:notify', source, {
                        title = 'Arme permanente',
                        description = 'Cette arme est permanente et ne peut pas être jetée',
                        type = 'error'
                ...
                if weapon.metadata and (
                    weapon.metadata.perm == true or 
                    weapon.metadata.permanent == true or 
                    weapon.metadata.permament == true or
                    weapon.metadata.isPermament == true or
                    weapon.metadata.isPermanent == true or
                    weapon.metadata.perm == 1 or
                    weapon.metadata.permanent == 1 or
                    weapon.metadata.permament == 1 or
                    weapon.metadata.isPermament == 1 or
                    weapon.metadata.isPermanent == 1
                ) then
                    TriggerClientEvent('ox_lib:notify', source, {
                        title = 'Arme permanente',
                        description = 'Cette arme est permanente et ne peut pas être jetée',
