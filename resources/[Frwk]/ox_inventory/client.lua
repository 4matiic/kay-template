...
-- Désactiver le système de crafting bench
...
function client.openInventory(inv, data)
	...
	if inv == 'dumpster' and cache.vehicle then
		...
		return ESX.ShowNotification("~r~Vous êtes occupé")
	end
    ...
    elseif inv == 'crafting' then
        return ESX.ShowNotification("~r~Les crafting benches sont désactivés")
    ...
    if inv == 'trunk' then
		...
		pedScreen = exports['ped_inventory']:PedScreenCreate({
...
function client.closeInventory(server)
	...
	if invOpen then
		...
		closeTrunk()
		if pedScreen then
			exports['ped_inventory']:PedScreenDelete(pedScreen)
			pedScreen = nil
		end
	...
	CreateThread(function()
		exports['ped_inventory']:PedScreenCreate({
			dict = "anim@amb@nightclub@peds@",
			anim = "rcmme_amanda1_stand_loop_cop"
		})
	end)
	...
	if invOpen then
		CreateThread(function()
			exports['ped_inventory']:PedScreenDelete()
		end)
...
RegisterNetEvent('ox_inventory:setPedComponent', function(components)
	local currentSkin = {}
	TriggerEvent('skinchanger:getSkin', function(skin)
		currentSkin = skin
	end)

	if components.component_id == 1 then -- mask
		currentSkin['mask_1'] = components.drawable
		currentSkin['mask_2'] = components.texture
	elseif components.component_id == 11 then -- jacket
		currentSkin['torso_1'] = components.drawable
		currentSkin['torso_2'] = components.texture
	elseif components.component_id == 8 then -- undershirt
		currentSkin['tshirt_1'] = components.drawable
		currentSkin['tshirt_2'] = components.texture
	elseif components.component_id == 9 then -- bodyarmor
		currentSkin['bproof_1'] = components.drawable
		currentSkin['bproof_2'] = components.texture
	elseif components.component_id == 4 then -- pants
		currentSkin['pants_1'] = components.drawable
		currentSkin['pants_2'] = components.texture
	elseif components.component_id == 6 then -- shoes
		currentSkin['shoes_1'] = components.drawable
		currentSkin['shoes_2'] = components.texture
	elseif components.component_id == 3 then -- arms/gloves
		currentSkin['arms'] = components.drawable
		currentSkin['arms_2'] = components.texture
	elseif components.component_id == 5 then -- bag
		currentSkin['bags_1'] = components.drawable
		currentSkin['bags_2'] = components.texture
	elseif components.component_id == 7 then -- chain
		currentSkin['chain_1'] = components.drawable
		currentSkin['chain_2'] = components.texture
	end

	TriggerEvent('skinchanger:loadSkin', currentSkin)
	TriggerServerEvent('esx_skin:save', currentSkin)

	exports['ped_inventory']:ApplyPedComponent(components)
end)

RegisterNetEvent('ox_inventory:setPedProp', function(props)
	local currentSkin = {}
	TriggerEvent('skinchanger:getSkin', function(skin)
		currentSkin = skin
	end)

	if props.prop_id == 0 then -- hat
		currentSkin['helmet_1'] = props.drawable
		currentSkin['helmet_2'] = props.texture
	elseif props.prop_id == 1 then -- glasses
		currentSkin['glasses_1'] = props.drawable
		currentSkin['glasses_2'] = props.texture
	elseif props.prop_id == 2 then -- earrings
		currentSkin['ears_1'] = props.drawable
		currentSkin['ears_2'] = props.texture
	elseif props.prop_id == 6 then -- watch
		currentSkin['watches_1'] = props.drawable
		currentSkin['watches_2'] = props.texture
	elseif props.prop_id == 7 then -- bracelet
		currentSkin['bracelets_1'] = props.drawable
		currentSkin['bracelets_2'] = props.texture
	end

	TriggerEvent('skinchanger:loadSkin', currentSkin)
	TriggerServerEvent('esx_skin:save', currentSkin)

	exports['ped_inventory']:ApplyPedProp(props)
end)

RegisterNetEvent('ox_inventory:refreshPed', function()
	if LocalPlayer.state.invOpen then
		if exports['ped_inventory']:IsPedScreenActive() then
			CreateThread(function()
				Wait(100)
				exports['ped_inventory']:SyncPedWithPlayer()
			end)
		else
			exports['ped_inventory']:ResetPedScreen()
		end
	end
end)

RegisterNetEvent('ox_inventory:previewClothing', function(itemData)
	if not itemData then
		exports['ped_inventory']:RestoreOriginalClothing()
		return
	end

	if not exports['ped_inventory']:IsPedScreenActive() then
		exports['ped_inventory']:ResetPedScreen()
		Wait(500)
	end

	exports['ped_inventory']:ApplyClothingPreview(itemData)
end)
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	local fromItem = PlayerData.inventory[data.fromSlot]
	local toItem = data.toSlot and PlayerData.inventory[data.toSlot]
	...
	local isFromEquipSlot = data.fromType == 'player' and data.fromSlot >= 31 and data.fromSlot <= 46
	local isToEquipSlot = data.toType == 'player' and data.toSlot >= 31 and data.toSlot <= 46
	...
	if fromItem and fromItem.name == 'cloth' then
		local success = lib.callback.await('ox_inventory:swapItems', false, data)
		
		if success then
			if fromItem.metadata then
			else
			end
			
			if isFromEquipSlot and not isToEquipSlot then
				local sex = IsPedModel(cache.ped, `mp_m_freemode_01`) and 'male' or 'female'
				local defaults = {
					male = {
				...
				for componentId, default in pairs(defaults[sex]) do
					SetPedComponentVariation(cache.ped, componentId, default.drawable, default.texture, 0)
					Wait(50)
				end
			...
			if not isToEquipSlot then
				swapActive = false
				return cb(success)
			end
			
			if fromItem.metadata.outfit then
				for i = 0, 11 do
					local compKey = 'comp' .. i
					local textureKey = 'comp' .. i .. '_2'
					if fromItem.metadata.outfit[compKey] ~= nil then
						local value = fromItem.metadata.outfit[compKey]
						local texture = fromItem.metadata.outfit[textureKey] or 0
						SetPedComponentVariation(PlayerPedId(), i, value, texture, 0)
						Wait(50)
					end
				end
			elseif fromItem.metadata.components then
				for type, selection in pairs(fromItem.metadata.components) do
					local componentId = nil
					if type == 'torso_1' then componentId = 11
					elseif type == 'tshirt_1' then componentId = 8
					elseif type == 'arms' then componentId = 3
					elseif type == 'pants_1' then componentId = 4
					elseif type == 'shoes_1' then componentId = 6
					elseif type == 'accessories' then componentId = 7
					end
					
					if componentId and selection.index > 0 then
						SetPedComponentVariation(PlayerPedId(), componentId, selection.index-1, selection.texture, 0)
						Wait(50)
					end
				end
			else
	...
	if fromItem and fromItem.name:match('^cloth_') then
		local success = lib.callback.await('ox_inventory:swapItems', false, data)
		...
		if success then
			...
			local componentId = fromItem.metadata.component
 
			if isFromEquipSlot and isToEquipSlot and toItem and fromItem.name == toItem.name then
				SetPedComponentVariation(
					cache.ped,
					componentId,
					toItem.metadata.drawable,
					toItem.metadata.texture,
					0
				)
			elseif isFromEquipSlot then
				local default = defaults[sex][componentId]
				if default then
					SetPedComponentVariation(cache.ped, componentId, default.drawable, default.texture, 0)
				end
			elseif isToEquipSlot then 
				SetPedComponentVariation(
					cache.ped,
					componentId,
					fromItem.metadata.drawable,
					fromItem.metadata.texture,
					0
				)
			end
		end
	...
	if fromItem and fromItem.name == 'cloth' and fromItem.metadata and fromItem.metadata.outfit then
		local success = lib.callback.await('ox_inventory:swapItems', false, data)
		
		if success then
			
			local outfit = fromItem.metadata.outfit
			local components = fromItem.metadata.components
				...
				for comp, value in pairs(outfit) do
					if comp:match('^comp') and not comp:match('_2$') then
						...
						if componentId then
							...
							SetPedComponentVariation(cache.ped, componentId, value, texture, 0)
							Wait(50)
						end
					end
				end
			elseif components then
				for type, selection in pairs(components) do
					if selection and selection.index > 0 then
						local componentId = nil
						...
						if componentId then
							SetPedComponentVariation(cache.ped, componentId, selection.index-1, selection.texture, 0)
							Wait(50) 
						end
					end
				end
 ...
 function GetComponentIdFromType(componentType)
	local componentMap = {
		['helmet_1'] = 0,
		['mask_1'] = 1,
		['hair_1'] = 2,
...
RegisterNUICallback('is_not_cloth', function(data, cb)
    ESX.ShowNotification("~r~Emplacement invalide !")
    cb(1)
end)
...
local isInNearbyMode = false
local nearbyUpdateThread = nil

RegisterNUICallback('getNearbyPlayers', function(data, cb)
    isInNearbyMode = true
    
    exports['modules']:NearbyPlayersClear()
    ...
    if #players > 0 then
        success = exports['modules']:NearbyPlayersDisplay(players)
    else
        -- ESX.ShowNotification("Aucun joueur à cote")
    end
    
    if nearbyUpdateThread == nil then
        nearbyUpdateThread = CreateThread(function()
            local lastPlayers = {}
            
            while isInNearbyMode do
                Wait(5000)
                
                if not isInNearbyMode then break end
                ...
                if #currentPlayers ~= #lastPlayers then
                    needsUpdate = true
                else
                    for i = 1, #currentPlayers do
                        local found = false
                        for j = 1, #lastPlayers do
                            if currentPlayers[i].id == lastPlayers[j].id then
                                found = true
                                break
                            end
                        end
                ...
                if needsUpdate then
                    ...
                    if #currentPlayers > 0 then
                        exports['modules']:NearbyPlayersDisplay(currentPlayers)
                    end
...
RegisterNUICallback('clearNearbyPlayers', function(_, cb)
...
RegisterNUICallback('showGroundItems', function(data, cb)
    exports['modules']:NearbyPlayersClear()
    cb({})
end)

RegisterNUICallback('showWeaponAttachments', function(data, cb)
	cb(1)
	
	
	if not data.slot or not data.name then 
		return 
	end
	
	local weaponData = data.name and Items[data.name]
	if not weaponData or not weaponData.weapon then 
		return 
	end
	
	local modulesilableAttachments = {}
	
	local attachmentTypes = {
		{ type = 'silencer', label = 'Silencieux' },
		{ type = 'flashlight', label = 'Lampe Tactique' },
		{ type = 'sight', label = 'Viseur' },
		{ type = 'grip', label = 'Poignée' },
		{ type = 'magazine', label = 'Chargeur' },
		{ type = 'barrel', label = 'Canon' },
		{ type = 'laser', label = 'Viseur Laser' },
		{ type = 'muzzle', label = 'Frein de Bouche' },
		{ type = 'skin', label = 'Apparence' }
	}
	
	local item = PlayerData.inventory[data.slot]
	local currentComponents = item?.metadata?.components or {}
	
	
	for _, attachType in ipairs(attachmentTypes) do
		local isCompatible = false
		local currentComponent
		
		for _, component in ipairs(currentComponents) do
			local componentData = Items[component]
			if componentData and componentData.type == attachType.type then
		...
		if not currentComponent then
			if (data.name:lower() == 'weapon_pistol' or data.name:lower() == 'weapon_combatpistol') and 
				(attachType.type == 'grip' or attachType.type == 'barrel') then
				isCompatible = false
			else
				isCompatible = true
			end
		...
		table.insert(modulesilableAttachments, {
			type = attachType.type,
			label = attachType.label,
	...
	SendNUIMessage({
		action = 'setupWeaponAttachments',
		data = {
			weaponSlot = data.slot,
			weaponName = data.name,
			attachments = modulesilableAttachments
		}
	})
end)

RegisterNUICallback('equipWeaponAttachment', function(data, cb)
	cb(1)
	
	if not data.weaponSlot or not data.componentSlot then return end
	
	local success = lib.callback.await('ox_inventory:equipWeaponComponent', false, data.weaponSlot, data.componentSlot)
	
	if success then
		TriggerEvent('inventory:client:refreshWeaponAttachments')
	end
end)
...
RegisterNUICallback('removeWeaponAttachment', function(data, cb)
	...
	if not data.weaponSlot or not data.componentIndex then return end
	
	local success = lib.callback.await('ox_inventory:removeWeaponComponent', false, data.weaponSlot, data.componentIndex)
...
function ApplyClothingAfterSwap()
    ...
    for slot = 31, 46 do
        local item = PlayerData.inventory[slot]
        if item then
            if item.name == 'cloth' and item.metadata then
                table.insert(clothingItems, item)
            elseif item.name:match('^cloth_') and item.metadata then
                table.insert(clothingItems, item)
            end
        end
    end
    ...
    for _, item in ipairs(clothingItems) do
        if item.name == 'cloth' then
            if item.metadata.outfit then
                for i = 0, 11 do
                    local compKey = 'comp' .. i
                    local textureKey = 'comp' .. i .. '_2'
                    if item.metadata.outfit[compKey] ~= nil then
                        local value = item.metadata.outfit[compKey]
                        local texture = item.metadata.outfit[textureKey] or 0
                        SetPedComponentVariation(PlayerPedId(), i, value, texture, 0)
                        Wait(50)
                    end
                end
            elseif item.metadata.components then
                for type, selection in pairs(item.metadata.components) do
                    local componentId = nil
                    if type == 'torso_1' then componentId = 11
                    elseif type == 'tshirt_1' then componentId = 8
                    elseif type == 'arms' then componentId = 3
        ...
        elseif item.name:match('^cloth_') then
            local componentId = item.metadata.component
            if componentId ~= nil then
                SetPedComponentVariation(
                    PlayerPedId(),
                    componentId,
                    item.metadata.drawable,
...
RegisterNUICallback('renameItem', function(data, cb)
    local controls = {30, 31, 32, 33, 34, 35, 36, 21, 22, 44, 37, 24, 25}
    ...
    local input = lib.inputDialog('Renommer', {
        { type = 'input', label = 'Nouvelle étiquette', placeholder = '' }
    })
    ...
    if input and input[1] then
        local label = input[1]
        lib.callback('ox_inventory:renameItem', false, function(success)
