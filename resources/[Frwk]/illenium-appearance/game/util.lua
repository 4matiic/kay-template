...
local function setPedFaceFeatures(ped, faceFeatures)
    ...
    for k, v in pairs(constants.FACE_FEATURES) do
        local value = faceFeatures[v]
        if type(value) == "number" then
            SetPedFaceFeature(ped, k-1, value + 0.0)
        end
    end
end
...
local function setPedHeadOverlays(ped, headOverlays)
    ...
    local overlayMap = {
        blemishes = {id = 0, colorType = 0},
        beard = {id = 1, colorType = 1},
        eyebrows = {id = 2, colorType = 1},
        ageing = {id = 3, colorType = 0},
        makeUp = {id = 4, colorType = 2},
        blush = {id = 5, colorType = 2},
        complexion = {id = 6, colorType = 0},
        sunDamage = {id = 7, colorType = 0},
        lipstick = {id = 8, colorType = 2},
        moleAndFreckles = {id = 9, colorType = 0},
        chestHair = {id = 10, colorType = 1},
        bodyBlemishes = {id = 11, colorType = 0}
    }
    
    for overlayName, data in pairs(overlayMap) do
        local overlay = headOverlays[overlayName]
        if overlay then
            SetPedHeadOverlay(ped, data.id, overlay.style or 0, tofloat(overlay.opacity or 1.0))
            
            if data.colorType > 0 and overlay.color then
                SetPedHeadOverlayColor(ped, data.id, data.colorType, overlay.color or 0, overlay.secondColor or 0)
            end
        else
            SetPedHeadOverlay(ped, data.id, 0, 0.0)
        end
    end
end
