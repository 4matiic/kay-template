...
function InitAppearance()
    if Framework.UpdatePlayerData then
...
function InitializeCharacter(gender, onSubmit, onCancel)
    ...
    client.startPlayerCustomization(function(appearance)
        if (appearance) then
            ...
            TriggerEvent('illenium-appearance:client:forceSave')
            if onSubmit then
                onSubmit()
            end
        ...
        client.startPlayerCustomization(function(appearance)
            if appearance then
                if not isPedMenu then
                    if Config.SaveClothesAsItems and shopType == "clothing" then
                        local newComponents = appearance.components or {}
                        local oldComponents = initialPedAppearance.components or {}
                        local changedItems = {}
                        ...
                        for _, newComp in ipairs(newComponents) do
                            ...
                            for _, oldComp in ipairs(oldComponents) do
                                if oldComp.component_id == newComp.component_id then
                                    found = true
                                    if oldComp.drawable ~= newComp.drawable or oldComp.texture ~= newComp.texture then
                                        table.insert(changedItems, {
                                            componentId = newComp.component_id, 
                                            drawable = newComp.drawable,
                                            texture = newComp.texture
                                        })
                                    end
                                    break
                                end
                        ...
                        local newProps = appearance.props or {}
                        local oldProps = initialPedAppearance.props or {}
                        ...
                        for _, newProp in ipairs(newProps) do
                            ...
                            for _, oldProp in ipairs(oldProps) do
                                   if oldProp.prop_id == newProp.prop_id then
                                    found = true
                                    if oldProp.drawable ~= newProp.drawable or oldProp.texture ~= newProp.texture then
                                        table.insert(changedItems, {
                                            propId = newProp.prop_id,
                                            drawable = newProp.drawable,
                                            texture = newProp.texture
                                        })
                                    end
                                    break
                                end
                        ...
                        if #changedItems > 0 then
                            local purchaseAppearance = appearance
                            
                            client.setPlayerAppearance(initialPedAppearance)
                            
                            TriggerServerEvent("illenium-appearance:server:saveClothingItems", purchaseAppearance, changedItems, initialPedAppearance)
                        else
...
local function RegisterChangeOutfitMenu(id, parent, outfits, mType)
    ...
    if outfits and #outfits > 0 then
        ...
        changeOutfitMenu.options[1] = {
            title = "No outfits available",
            description = "Save an outfit first",
            disabled = true
        }
    end
...
local function RegisterGenerateOutfitCodeMenu(id, parent, outfits)
    ...
    if outfits and #outfits > 0 then
        ...
        generateOutfitCodeMenu.options[1] = {
            title = "No outfits available",
            description = "Save an outfit first",
            disabled = true
        }
