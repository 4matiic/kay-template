...
RegisterNUICallback('ready', function(data, cb)
    repeat Wait(0) until FB
    TriggerServerEvent('ava:getTime')
    cb({
        isFA = IS_FA,
        isRDR2 = IS_RDR2,
        moneyDm = MONEY_DM,
...
RegisterNetEvent('ava:time', function(time)
...
AddEventHandler('ava:focus', function(actType, ref)
...
RegisterCommand('restartui', function()
	...
	SetNuiFocusKeepInput(false)
    SetNuiFocus(false, false)
    TriggerEvent('ava:restartui')
end, true)


local rateLimiters = {}

function RateLimiter(key, limit, timeFrame)
    ...
    if currentTime - limiter.lastReset >= timeFrame then
        limiter.count = 0
