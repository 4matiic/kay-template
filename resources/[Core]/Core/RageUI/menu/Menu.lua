function RageUI.CreateMenu(Title, Subtitle, X, Y, TextureDictionary, TextureName, R, G, B, A)
    ...
    Menu.TitleFont = 6
    Menu.TitleScale = 0.6
    Menu.Subtitle = Subtitle and string.upper(Subtitle) or nil
...
function RageUI.CreateSubMenu(ParentMenu, Title, Subtitle, X, Y, TextureDictionary, TextureName, R, G, B, A)
    if ParentMenu ~= nil then
        ...
        local fullTitle = Title or parent.Title or ""
        local menu = RageUI.CreateMenu(fullTitle, Subtitle and string.upper(Subtitle) or parent.Subtitle, X or parent.X, Y or parent.Y)
        menu.Parent = ParentMenu
        menu.WidthOffset = parent.WidthOffset
        menu.Safezone = parent.Safezone

        if parent.Sprite then
            menu.Sprite = {
                Dictionary = TextureDictionary or parent.Sprite.Dictionary,
                Texture = TextureName or parent.Sprite.Texture,
                Color = {
                    R = R or parent.Sprite.Color.R,
                    G = G or parent.Sprite.Color.G,
                    B = B or parent.Sprite.Color.B,
                    A = A or parent.Sprite.Color.A
                }
            }
        else
            menu.Rectangle = parent.Rectangle
        end
...
function RageUI.CreateSubMenuStaff(ParentMenu, Title, Subtitle, X, Y, TextureDictionary, TextureName, R, G, B, A)
    if ParentMenu ~= nil then
        if ParentMenu() then
            local Menu = RageUI.CreateMenu(Title or ParentMenu.Title, Subtitle and string.upper(Subtitle) or string.upper(ParentMenu.Subtitle),  1400, 110)
            Menu.Parent = ParentMenu
